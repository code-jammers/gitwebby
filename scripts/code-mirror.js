...
let StringStream_default = StringStream;
  ...
  if (boxAround) {
    ...
    sticky =
      ch == 0
        ...
        : ch == lineObj.text.length
        ? 'before'
        : measureCharPrepared(cm, preparedMeasure, ch - (ltr ? 1 : 0)).bottom + widgetHeight2 <= y == ltr
        ? 'after'
        : 'before';
...
function updateDisplayIfNeeded(cm, update) {
  ...
  let different =
    from != display.viewFrom || to != display.viewTo || display.lastWrapHeight != update.wrapperHeight || display.lastWrapWidth != update.wrapperWidth;
  ...
  if (
    (hist.lastOp == opId ||
      (hist.lastOrigin == change.origin &&
        change.origin &&
        ((change.origin.charAt(0) == '+' && hist.lastModTime > time - (doc.cm ? doc.cm.options.historyEventDelay : 500)) || change.origin.charAt(0) == '*'))) &&
...
function addSelectionToHistory(doc, sel, opId, options) {
  ...
  if (
    ...
    (origin &&
      ...
      ((hist.lastModTime == hist.lastSelTime && hist.lastOrigin == origin) || selectionEventCanBeMerged(doc, origin, lst(hist.done), sel)))
  ) {
...
function shiftDoc(doc, distance) {
  ...
  doc.sel = new Selection(
    map(doc.sel.ranges, range2 => new Range(Pos(range2.anchor.line + distance, range2.anchor.ch), Pos(range2.head.line + distance, range2.head.ch))),
    doc.sel.primIndex
  );
  if (doc.cm) {
    ...
    this.iter(from.line, to.line + 1, line => {
      ...
      if (spans) {
        for (let i = 0; i < spans.length; i++) {
          ...
          if (
            !(
              (span.to != null && lineNo2 == from.line && from.ch >= span.to) ||
              (span.from == null && lineNo2 != from.line) ||
              (span.from != null && lineNo2 == to.line && span.from >= to.ch)
            ) &&
...
function defineOptions(CodeMirror2) {
  ...
  function option(name, deflt, handle, notOnInit) {
    ...
    if (handle) {
      optionHandlers2[name] = notOnInit
        ? (cm, val, old) => {
            if (old != Init) {
              handle(cm, val, old);
            }
...
function methods_default(CodeMirror2) {
  ...
  CodeMirror2.prototype = {
    ...
    heightAtLine: function (line, mode, includeWidgets) {
      ...
      return (
        intoCoordSystem(this, lineObj, { top: 0, left: 0 }, mode || 'page', includeWidgets || end).top + (end ? this.doc.height - heightAtLine(lineObj) : 0)
      );
    },
...
let ContentEditableInput = class {
  constructor(cm) {
    this.cm = cm;
...
let ContentEditableInput_default = ContentEditableInput;
...
let TextareaInput_default = TextareaInput;
...
let codemirror_default = CodeMirror;
