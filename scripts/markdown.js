...
let global$1 = typeof global !== 'undefined' ? global : typeof self !== 'undefined' ? self : typeof window !== 'undefined' ? window : {};
let _self = {};
let Prism = (function () {
  ...
  var _ = (_self.Prism = {
    ...
    languages: {
      extend(id, redef) {
        let lang2 = _.util.clone(_.languages[id]);
    ...
    highlightAllUnder(container, async, callback) {
      ...
      let elements = env.elements || container.querySelectorAll(env.selector);
      ...
      for (let token in grammar) {
        ...
        for (let j = 0; j < patterns.length; ++j) {
          ...
          for (let i = index, pos = startPos; i < strarr.length; pos += strarr[i].length, ++i) {
            ...
            let wrapped = new Token2(token, inside ? _.tokenize(match, inside) : match, alias, match, greedy);
            args.push(wrapped);
            if (after) {
              args.push(after);
            }
...
let prism_default = Prism;
function createCommonjsModule(fn) {
  let module2 = { exports: {} };
  return fn(module2, module2.exports), module2.exports;
}
let defaults$5 = createCommonjsModule(function (module2) {
...
let escapeTest = /[&<>"']/;
let escapeReplace = /[&<>"']/g;
let escapeTestNoEncode = /[<>"']|&(?!#?\w+;)/;
let escapeReplaceNoEncode = /[<>"']|&(?!#?\w+;)/g;
...
let unescapeTest = /&(#(?:\d+)|(?:#x[0-9A-Fa-f]+)|(?:\w+));?/gi;
...
let noopTest$1 = { exec: function noopTest() {} };
...
let { defaults: defaults$4 } = defaults$5;
let { rtrim, splitCells, escape: escape$2, findClosingBracket } = helpers;
...
let Tokenizer_1 = class Tokenizer {
  constructor(options) {
...
let { noopTest: noopTest2, edit, merge: merge$1 } = helpers;
let block$1 = {
  newline: /^(?: *(?:\n|$))+/,
  code: /^( {4}[^\n]+(?:\n(?: *(?:\n|$))*)?)+/,
  fences: /^ {0,3}(`{3,}(?=[^`\n]*\n)|~{3,})([^\n]*)\n(?:|([\s\S]*?)\n)(?: {0,3}\1[~`]* *(?:\n+|$)|$)/,
...
let { defaults: defaults$3 } = defaults$5;
let { block, inline } = rules;
let { repeatString } = helpers;
...
let Lexer_1 = class Lexer {
  ...
  inlineTokens(src, tokens = [], inLink = false, inRawBlock = false) {
    ...
    while ((match = this.tokenizer.rules.inline.blockSkip.exec(maskedSrc)) != null) {
      maskedSrc =
        maskedSrc.slice(0, match.index) + '[' + repeatString('a', match[0].length - 2) + ']' + maskedSrc.slice(this.tokenizer.rules.inline.blockSkip.lastIndex);
    }
...
let { defaults: defaults$2 } = defaults$5;
let { cleanUrl, escape: escape$1 } = helpers;
let Renderer_1 = class Renderer {
  constructor(options) {
...
let TextRenderer_1 = class TextRenderer {
  strong(text) {
    return text;
  }
  em(text) {
...
let { defaults: defaults$1 } = defaults$5;
let { unescape } = helpers;
let Parser_1 = class Parser {
  constructor(options) {
...
let { merge, checkSanitizeDeprecation, escape } = helpers;
let { getDefaults, changeDefaults, defaults } = defaults$5;
...
marked.parse = marked;
let marked_1 = marked;
let marked_esm_default = marked_1;
